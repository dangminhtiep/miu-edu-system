...
export function HomeworkAiWorkspace({
  ...
  async function postForm(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error ?? "Khong the xu ly yeu cau.");
      }
        ...
        <div style={rowStyle}>
          <div style={{ maxWidth: 760 }}>
            ...
            <p style={{ margin: 0, lineHeight: 1.6, color: "#d7e4f2" }}>
              Giáo viên tạo bài, nhập rubric và đáp án mẫu. Học sinh chọn bài của lớp mình, nộp ảnh bài làm, xem kết quả AI nếu không bị gắn cờ rà soát và có quyền khiếu nại.
            </p>
          </div>
      ...
      {view === "teacher" ? (
        <section style={{ display: "grid", gap: 24, gridTemplateColumns: "minmax(340px, 430px) minmax(0, 1fr)" }}>
          ...
          >
            ...
            <Field label="Lớp">
              <input name="classCode" required style={inputStyle} />
            </Field>
            <Field label="Tiêu đề bài tập">
              <input name="title" required style={inputStyle} />
            </Field>
            <Field label="Đề bài">
              <textarea name="instructions" rows={4} required style={textareaStyle} />
            </Field>
            <Field label="Đáp án mẫu">
              <textarea name="answerKey" rows={4} required style={textareaStyle} />
            </Field>
          ...
          <div style={{ display: "grid", gap: 24 }}>
            <div style={panelStyle}>
              ...
              {assignments.length === 0 ? (
                <div style={emptyStyle}>Chưa có bài nào được giao.</div>
              ) : (
                assignments.map((assignment) => (
                  <article key={assignment.id} style={cardStyle}>
                    ...
                    <div>
                      Nhận bài: {new Date(assignment.startsAt).toLocaleString("vi-VN")}
                    </div>
                    <div>
                    ...
                    <div style={rowStyle}>
                      <div>
                        ...
                        <div style={{ color: "#5b6470" }}>
                          ...
                          {complaint.category === "wrong_submission" ? "Nộp nhầm bài" : "Khiếu nại chấm điểm"}
                        </div>
                      </div>
          ...
          <div style={panelStyle}>
            ...
            <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(2, minmax(0, 1fr))" }}>
              <Field label="Tên học sinh">
                <input value={studentName} onChange={(event) => setStudentName(event.target.value)} style={inputStyle} />
              </Field>
              <Field label="Lớp">
                <input value={classCode} onChange={(event) => setClassCode(event.target.value)} style={inputStyle} />
              </Field>
            </div>
          </div>
                    ...
                    <div style={{ display: "grid", gap: 14 }}>
                      <div style={cardStyle}>
                        ...
                        {latestSubmission.status === "graded" && latestSubmission.result ? (
                          <>
                            <div>
                              <strong>Điểm toàn bài:</strong>{" "}
                              {latestSubmission.result.totalScore}/
                              {latestSubmission.result.maxScore}
                            </div>
                            <div>
                              <strong>Lời khen:</strong> {latestSubmission.result.praise}
                            </div>
                            <div>
                              <strong>Lỗi sai cần lưu ý:</strong>
                              <ul style={listStyle}>
                                {latestSubmission.result.mistakesToNote.map((item) => (
                                  <li key={item}>{item}</li>
                                ))}
                              </ul>
                            </div>
                            <div>
                              ...
                              <ul style={listStyle}>
                                {latestSubmission.result.improvementSuggestions.map((item) => (
                                  <li key={item}>{item}</li>
                                ))}
                              </ul>
                            </div>
                          ...
                          <div>
                            MIU đã nhận bài. Nếu bài đang cần rà soát, kết quả chi tiết sẽ hiển thị sau khi giáo viên kiểm tra.
                          </div>
                        )}
                      </div>
